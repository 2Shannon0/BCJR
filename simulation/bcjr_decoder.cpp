...
vector<double> decode(
    ...
    vector<vector<tuple<string, double, string>>> gammas;

    for (const auto& layer : edg_bpsk) {
        vector<tuple<string, double, string>> new_layer;
    ...
    vector<unordered_map<string, double>> alphas(gammas.size() + 1);
    tuple<string, double, string> first_edge = gammas[0][0];
    ...
    for (size_t i = 0; i < gammas.size(); i++) {
        for (size_t j = 0; j < gammas[i].size(); j++) {
            ...
            double diff = pow(llr_in[i] - edge_value, 2) / (2 * sigma2);
            double cur_gamma = constant_coef * exp(-diff);

            gammas[i][j] = make_tuple(prev_vex, cur_gamma, next_vex);
            double new_alpha = cur_gamma * alphas[i][prev_vex];
    ...
    vector<unordered_map<string, double>> betas(gammas.size() + 1);
    ...
    for (int i = gammas.size() - 1; i >= 0; i--) {
        ...
        for (size_t j = 0; j < gammas[i].size(); j++) {
            ...
            double cur_gamma = get<1>(gammas[i][j]);
            string prev_vex = get<2>(gammas[i][j]);

            double new_beta = cur_gamma * betas[i + 1][prev_vex];
            betas[i][next_vex] += new_beta;

            double cur_alpha = alphas[i][next_vex];
            double cur_beta = betas[i + 1][prev_vex];
            double cur_sigma = cur_gamma * cur_alpha * cur_beta;
